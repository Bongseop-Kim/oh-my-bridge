...
## 부록: Superpowers의 토큰 비용 문제와 oh-my-bridge

최근 [Superpowers 플러그인 리뷰](https://www.youtube.com/watch?v=0oNMR8CGGJ8)가 화제가 됐다. 1년 동안 미뤄왔던 콘텐츠 파이프라인을 3시간 만에 완성했다는 사례와 함께, 한 가지 뚜렷한 단점이 지적됐다.

> *"Superpowers의 가장 큰 문제점은 과도한 토큰 소비량입니다. 모든 것이 동일한 고가의 모델에서 실행되기 때문입니다. 향후 개선점으로 모델 티어링 기능이 필요합니다."*

Superpowers는 브레인스토밍, 계획, 실행 각 단계를 별도 하위 에이전트로 분리한다. 그런데 이 하위 에이전트들이 모두 오케스트레이터와 동일한 모델(Opus급)에서 실행된다. HTML 가져오기나 라이브러리 추출처럼 단순한 작업에도 최고가 모델이 투입되는 것이다.

리뷰어는 단계별 적정 모델 티어를 이렇게 제안했다:

```
뉴스레터 가져오기, 라이브러리 추출 → Haiku
연구 에이전트                        → Sonnet
기사 작성                            → Opus
```

**oh-my-bridge를 함께 사용하면 이 문제를 부분적으로 해소할 수 있다.**

아키텍처가 맞닿아 있기 때문이다. Claude는 오케스트레이터로 전체 흐름을 지휘하고, 실제 코드 생성은 Codex(GPT 계열)가 담당한다. 코드 생성 단계만큼은 Claude 토큰을 쓰지 않아도 된다.

```
Superpowers (단점):
  브레인스토밍 에이전트  → Opus
  계획 에이전트          → Opus  ← 낭비
  HTML 파싱 에이전트     → Opus  ← 낭비
  코드 생성 에이전트     → Opus  ← 낭비

Superpowers + oh-my-bridge:
  브레인스토밍 에이전트  → Opus
  계획 에이전트          → Opus
  HTML 파싱 에이전트     → Opus
  코드 생성              → Codex CLI (GPT-4o 기반)  ← Claude 토큰 절약
```

물론 완전한 해결은 아니다. Superpowers의 하위 에이전트는 리서치, 파이프라인 실행 등 범용 작업을 담당하지만, oh-my-bridge의 Codex는 코드 생성에만 특화되어 있다. HTML 가져오기나 라이브러리 추출처럼 코드 생성 이외의 단순 작업에는 여전히 Opus가 투입된다.

그러나 **코드 생성 단계의 비용**만큼은, oh-my-bridge를 함께 사용하면 줄일 수 있다.
