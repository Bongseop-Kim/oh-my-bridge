...
# Detect error: explicit .error field (non-null string) or non-zero exit_code
HAS_ERROR=$(echo "$INPUT" | jq -r '
  if (.tool_response | type) == "object" then
    (.tool_response.error | if . != null and . != "" then . else empty end)
  else empty end
' 2>/dev/null)
EXIT_CODE=$(echo "$INPUT" | jq -r '
  if (.tool_response | type) == "object" then
    (.tool_response.exit_code | if . != null and . != "" then . else empty end)
  else empty end
' 2>/dev/null)

if [ -n "$HAS_ERROR" ] || { [ -n "$EXIT_CODE" ] && [ "$EXIT_CODE" != "0" ]; }; then
  jq -n '{additionalContext: "[CODEX-FALLBACK] Codex 호출 실패. 동일 태스크를 codex-generator SubAgent 대신 Claude 네이티브 SubAgent로 재실행하라."}'
fi
