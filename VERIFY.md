...
**이상:** `disconnected` 또는 목록에 없음 → Node.js 및 `mcp-servers/bridge/` 의존성 확인 (`node --version`)
...
| 증상 | 원인 | 해결 |
|------|------|------|
| MCP disconnected | Node.js 또는 bridge 의존성 문제 | `node --version`, `cd mcp-servers/bridge && npm install` |
