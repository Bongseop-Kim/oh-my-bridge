...
**이상:** `disconnected` 또는 목록에 없음 → `.mcp.json`의 바이너리 경로 확인, 필요 시 재빌드 (`cd mcp-servers/bridge && CGO_ENABLED=0 go build -o oh-my-bridge .`)
...
| 증상 | 원인 | 해결 |
|------|------|------|
| MCP disconnected | 바이너리 없거나 경로 오류 | `.mcp.json` 확인, `cd mcp-servers/bridge && CGO_ENABLED=0 go build -o oh-my-bridge .` |
