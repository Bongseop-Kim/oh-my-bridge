...
[![GitHub Release](https://img.shields.io/github/v/release/gimbongseob/oh-my-bridge?color=369eff&labelColor=black&logo=github&style=flat-square)](https://github.com/gimbongseob/oh-my-bridge/releases)
...
| MCP 서버 | 커버 모델 | 방식 |
|---------|---------|------|
| **Codex CLI** | GPT-5.3 Codex, GPT-5.4, GPT-5-Nano | OpenAI 공식 |
| **Gemini CLI** | Gemini Pro, Gemini Flash | 프로젝트 로컬 MCP 브리지 |
...
아래 항목 모두 `✔ connected` 확인:
- `plugin:oh-my-bridge:codex`
- `plugin:oh-my-bridge:gemini`
...
2. `mcp__plugin_oh-my-bridge_codex__codex` 호출
...
```bash
# 로컬 Gemini MCP 의존성 설치
cd mcp-servers/gemini && npm install
